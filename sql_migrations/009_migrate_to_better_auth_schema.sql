-- Migration: 009_migrate_to_better_auth_schema
-- Description: Updates table schemas to use TEXT for user_id and updates submit_trade function.

BEGIN;

-- 1. Alter Tables to support TEXT user_id
-- We use a SAFE cast. If it fails, existing UUIDs will be converted to string.
ALTER TABLE trades ALTER COLUMN user_id TYPE TEXT;
ALTER TABLE rules ALTER COLUMN user_id TYPE TEXT;
ALTER TABLE daily_locks ALTER COLUMN user_id TYPE TEXT;
ALTER TABLE violations ALTER COLUMN user_id TYPE TEXT;

-- 2. Drop old function to cleanly replace it (signature change)
DROP FUNCTION IF EXISTS submit_trade(text, text, numeric, numeric, numeric, numeric, numeric, numeric, text, time);
DROP FUNCTION IF EXISTS submit_trade(uuid, text, numeric, numeric, numeric, numeric, numeric, numeric, text, time);
-- Drop any other variants just in case
DROP FUNCTION IF EXISTS submit_trade(text, text, text, numeric, numeric, numeric, numeric, numeric, numeric, text, time);

-- 3. Re-create submit_trade with TEXT user_id support
...
BEGIN
    ...
    FROM trades 
    WHERE user_id = v_user_id AND executed_at::date = CURRENT_DATE;
    ...
    -- Max Loss Check (After Insert)
    IF p_status = 'CLOSED' AND p_pnl < 0 THEN
         -- Re-calculate daily PnL including this trade
         SELECT COALESCE(SUM(pnl), 0) INTO v_current_pnl
         FROM trades 
         WHERE user_id = v_user_id AND executed_at::date = CURRENT_DATE;
